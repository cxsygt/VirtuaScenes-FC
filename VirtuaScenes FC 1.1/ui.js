...
const UI = {
    ...
    renderField() {
        ...
        if (Game.actionMode === 'pass' && Game.selectedPlayer) {
            ...
            for (const t of targets) {
                ...
                // 成功率颜色：≥70绿，≥50黄，<50红
                const rate = t.successRate || 50;
                const lineColor = rate >= 70 ? 'rgba(78, 204, 163, 0.6)' :
                                  (rate >= 50 ? 'rgba(240, 165, 0, 0.5)' :
                                               'rgba(233, 69, 96, 0.5)');
                if (t.affordable) {
                    // 可传：高亮目标
                    ctx.fillStyle = rate >= 70 ? 'rgba(78, 204, 163, 0.25)' :
                                   (rate >= 50 ? 'rgba(240, 165, 0, 0.2)' :
                                                 'rgba(233, 69, 96, 0.2)');
                    ctx.fillRect(s.x, s.y, cs, cs);
                    ctx.strokeStyle = rate >= 70 ? '#4ecca3' :
                                     (rate >= 50 ? '#f0a500' : '#e94560');
                    ctx.lineWidth = 2;
                    ctx.strokeRect(s.x + 1, s.y + 1, cs - 2, cs - 2);
                    // 标注AP消耗和成功率
                    ctx.fillStyle = rate >= 70 ? '#4ecca3' :
                                   (rate >= 50 ? '#f0a500' : '#e94560');
                    ctx.font = 'bold 10px sans-serif';
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'top';
                    ...
                    ctx.fillText(rate + '%', s.x + cs/2, s.y + cs - 1);
                    ...
                    if (t.interceptors && t.interceptors.length > 0) {
                        for (const ic of t.interceptors) {
                            const is = this.gridToScreen(ic.x, ic.y);
                            // 红色叉标记阻挡
                            ctx.strokeStyle = 'rgba(233, 69, 96, 0.8)';
                            ctx.lineWidth = 2;
                            ctx.beginPath();
                            ctx.moveTo(is.x + 4, is.y + 4);
                            ctx.lineTo(is.x + cs - 4, is.y + cs - 4);
                            ctx.moveTo(is.x + cs - 4, is.y + 4);
                            ctx.lineTo(is.x + 4, is.y + cs - 4);
                            ctx.stroke();
                        }
                    }
                } else {
        ...
        // 高亮：射门目标
        if (Game.actionMode === 'shoot' && Game.selectedPlayer) {
            const shooter = Game.selectedPlayer;
            const goal = eng.getGoalRange(shooter.teamId);
            const goalCenter = eng.getGoalCenter(shooter.teamId);
            const shooterScreen = this.gridToScreen(shooter.x, shooter.y);

            // 球门每个格子按位置难度显示不同颜色
            for (let y = goal.yStart; y <= goal.yEnd; y++) {
                const analysis = eng.getShootTargetAnalysis(shooter, goal.x, y);
                const s = this.gridToScreen(goal.x, y);
                if (analysis) {
                    const rate = analysis.successRate;
                    // 颜色按成功率：≥70绿，≥50黄，<50红
                    const fillColor = rate >= 70 ? 'rgba(78, 204, 163, 0.35)' :
                                    (rate >= 50 ? 'rgba(240, 165, 0, 0.3)' :
                                                  'rgba(233, 69, 96, 0.3)');
                    const strokeColor = rate >= 70 ? '#4ecca3' :
                                       (rate >= 50 ? '#f0a500' : '#e94560');
                    ...
                    ctx.fillText(rate + '%', s.x + cs/2, s.y + cs/2);

                    // 画射门虚线
                    ctx.strokeStyle = strokeColor.replace('rgb', 'rgba').replace(')', ', 0.5)');
                    if (rate >= 70) ctx.strokeStyle = 'rgba(78, 204, 163, 0.5)';
                    else if (rate >= 50) ctx.strokeStyle = 'rgba(240, 165, 0, 0.5)';
                    else ctx.strokeStyle = 'rgba(233, 69, 96, 0.5)';
                    ctx.lineWidth = 2;
                    ctx.setLineDash([5, 4]);
                    ctx.beginPath();
                    ctx.moveTo(shooterScreen.x + cs/2, shooterScreen.y + cs/2);
                    ctx.lineTo(s.x + cs/2, s.y + cs/2);
                    ctx.stroke();
                    ctx.setLineDash([]);
                    ...
                    if (analysis.blockers && analysis.blockers.length > 0) {
                        for (const b of analysis.blockers) {
                            const bs = this.gridToScreen(b.x, b.y);
                            ctx.strokeStyle = 'rgba(233, 69, 96, 0.9)';
                            ctx.lineWidth = 3;
                            ctx.beginPath();
                            ctx.moveTo(bs.x + 4, bs.y + 4);
                            ctx.lineTo(bs.x + cs - 4, bs.y + cs - 4);
                            ctx.moveTo(bs.x + cs - 4, bs.y + 4);
                            ctx.lineTo(bs.x + 4, bs.y + cs - 4);
                            ctx.stroke();
                        }
                    }
                }
    ...
    showPlayerCard(p) {
        ...
        // 射门分析：显示球门每个位置的成功率
        if (!p.isGoalkeeper && p.hasBall && isCurrent) {
            const goal = eng.getGoalRange(p.teamId);
            const goalCenter = eng.getGoalCenter(p.teamId);
            html += '<div style="margin-top:8px;font-size:12px;color:#a0a0b0;border-top:1px solid #333;padding-top:6px">';
            html += '射门分析 (距球门' + Math.abs(goal.x - p.x) + '格):';
            html += '</div>';
            html += '<div style="display:flex;gap:4px;margin-top:4px;flex-wrap:wrap;">';
            for (let y = goal.yStart; y <= goal.yEnd; y++) {
                const analysis = eng.analyzeShoot(p, goal.x, y);
                const rate = analysis.successRate;
                const color = rate >= 70 ? '#4ecca3' : (rate >= 50 ? '#f0a500' : '#e94560');
                const posLabel = (y === goalCenter.y) ? '中' : (y < goalCenter.y ? '上' : '下');
                const blockNote = analysis.blockers.length > 0 ? '⚠' + analysis.blockers.length : '';
                html += '<div style="font-size:11px;color:' + color + ';padding:3px 8px;background:#1a1a4e;border-radius:4px;border:1px solid ' + color + '">';
                html += posLabel + ' ' + rate + '%' + (blockNote ? ' ' + blockNote : '');
                html += '</div>';
            }
            html += '</div>';
            if (ap < 3) html += '<div style="font-size:11px;color:#888;">(AP不足射门)</div>';
        }
        ...
        if (p.hasBall && isCurrent) {
            ...
            if (passTargets.length > 0) {
                // 按成功率排序
                passTargets.sort((a, b) => (b.successRate || 0) - (a.successRate || 0));
                html += '<div style="margin-top:8px;font-size:12px;color:#a0a0b0;border-top:1px solid #333;padding-top:6px">';
                html += '可传球目标:';
                html += '</div>';
                for (const t of passTargets) {
                    const rate = t.successRate || 0;
                    const color = !t.affordable ? '#888' :
                                 (rate >= 70 ? '#4ecca3' :
                                 (rate >= 50 ? '#f0a500' : '#e94560'));
                    const strike = t.affordable ? '' : 'text-decoration:line-through;opacity:0.5;';
                    const blockNote = (t.interceptors && t.interceptors.length > 0) ? ' ⚠' + t.interceptors.length : '';
